...
public:
  ...
  {
  	if(tr.size()==0) {cout<<"Tree is currently empty\n"; return;}
  	cout<<"Current transaction array is: \n";
...
{
	cout<<"****\n";
	cout<<"1 to insert a new transaction\n";
	cout<<"2 to show the hashed root\n";
	cout<<"3 to show the transactions array\n";
...
int32_t main()
{
	...
	{
		x=instructions();
		if(x==1) M_Tree.insert();
		else if(x==2) M_Tree.print_root();
		else if(x==3) M_Tree.print_transactions();
		else if(x==4) M_Tree.print_merkle_tree();
		else if(x==5) exit(0);
	}
}